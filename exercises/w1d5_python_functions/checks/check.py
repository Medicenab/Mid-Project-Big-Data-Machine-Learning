...
def test_mayor(fn):
    class TestKnown(unittest.TestCase):
        ...
        def runTest(self):
            self.assertEqual(fn(*self.input), self.output, f"Answer should be {self.output}")
...
def test_unicos(fn):
    class TestKnown(unittest.TestCase):
        ...
        def runTest(self):
            stud_ans = fn(self.input)
            self.assertIsInstance(stud_ans, list, "Output should be `list` object.")
            self.assertEqual(sorted(stud_ans), self.output, f"Answer should be {self.output}")
    suite = unittest.TestSuite()
    for _ in range(100):
        arr = [random.randint(-100,100) for _ in range(random.randint(10,1000))]
        suite.addTest(TestKnown(arr,sorted(set(arr))))
    unittest.TextTestRunner().run(suite)

def test_calcula_moda(fn):
    class TestKnown(unittest.TestCase):
        def __init__(self, input, output):
            super(TestKnown, self).__init__()
            self.input = input
...
def test_stdev(fn):
    class TestKnown(unittest.TestCase):
        ...
        def runTest(self):
            self.assertAlmostEqual(fn(self.input), self.output, delta=5, msg=f"Answer should be {self.output}")
    suite = unittest.TestSuite()
    for _ in range(100):
        arr = [random.randint(-1000,1000) for _ in range(random.randint(10,100))]
...
"The$%&$%&, quick brown fooooooooxxx jumpssss oveeeer the laaaaaaazy12345678901234567890 dog!!!!!!",
...
def test_pangram(fn):
    class TestKnown(unittest.TestCase):
        ...
        def runTest(self):
            self.assertEqual(fn(self.input), self.output, f"Answer should be {self.output}")
    suite = unittest.TestSuite()
    tests = pangrams + ["".join([random.choice(ascii_lowercase+"0123456789!·$%&/()=., ?¿") for _ in range(random.randint(10,250))]) for _ in range(100)] 
    for test in tests:
